    ...
    I: IIndent,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let mut queue: VecDeque<(&T, Layer, String)> = VecDeque::new();
        enqueue(&mut queue, self.tree, String::new());
        while let Some((leaf, layer, s)) = queue.pop_front() {
            ...
            if !leaf.leaves().is_empty() {
                ...
                enqueue(&mut queue, leaf, leave_spaces);
            }
        }
        Ok(())
    }
...
fn enqueue<'a, T>(queue: &mut VecDeque<(&'a T, Layer, String)>, tree: &'a T, spaces: String)
where
    T: ITree<Leave = T>,
{
