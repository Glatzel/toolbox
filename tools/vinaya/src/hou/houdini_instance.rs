...
mod test {
    ...
    pub struct FakeHoudiniInstanceDir {
        pub path_99_0_456: PathBuf,
        pub path_99_5_123: PathBuf,
        pub path_10_0_456: PathBuf,
    }

    impl FakeHoudiniInstanceDir {
        pub fn new() -> mischief::Result<Self> {
            let path_99_0_456 = PathBuf::from(format!("{}", super::HoudiniInstance::INSTALL_DIR))
                .join("Houdini 99.0.456");
            let path_99_5_123 = PathBuf::from(format!("{}", super::HoudiniInstance::INSTALL_DIR))
                .join("Houdini 99.5.123");
            let path_10_0_456 = PathBuf::from(format!("{}", super::HoudiniInstance::INSTALL_DIR))
                .join("Houdini 10.0.456");

            fs::create_dir_all(&path_99_0_456).into_mischief()?;
            fs::create_dir_all(&path_99_5_123).into_mischief()?;
            fs::create_dir_all(&path_10_0_456).into_mischief()?;

            fs::create_dir_all(path_99_0_456.join("toolkit").join("cmake")).into_mischief()?;
            fs::create_dir_all(path_99_5_123.join("toolkit").join("cmake")).into_mischief()?;
            fs::create_dir_all(path_10_0_456.join("toolkit").join("cmake")).into_mischief()?;
        ...
        pub fn path_99_0_456(&self) -> &Path { &self.path_99_0_456 }
        pub fn path_99_5_123(&self) -> &Path { &self.path_99_5_123 }
        pub fn path_10_0_456(&self) -> &Path { &self.path_10_0_456 }
    }

    impl Drop for FakeHoudiniInstanceDir {
        fn drop(&mut self) {
            let _ = fs::remove_dir_all(&self.path_99_0_456);
            let _ = fs::remove_dir_all(&self.path_99_5_123);
            let _ = fs::remove_dir_all(&self.path_99_5_123);
        }
    }
    #[test]
    fn test_fake_dir() -> mischief::Result<()> {
        let path_10_0_456 = PathBuf::from(format!("{}", super::HoudiniInstance::INSTALL_DIR))
            .join("Houdini 10.0.456");
        assert!(!path_10_0_456.exists());
        let _ = FakeHoudiniInstanceDir::new()?;
        assert!(path_10_0_456.exists());
        Ok(())
    }
}
